 ...
 * it gives to the semaphore1 and gives to semaphore2 if its not in the sequence ,  if semaphore1 is more than 0
 * uart transmitter1 takes it and sends "TRUE"
 * and if semaphore2 is more than 0 sends "-"
 *
 * and our shared uart periph usart2 is protected with a mutex semaphore
 *
*/
...
void Simple_Transmitter2(void* params);
...
int main(void)
{
  ...
  xTaskCreate(Simple_Transmitter2 , "Simple task to transmit -  " , SIMPLE_TASK_STACK_SIZE ,NULL , 2 , NULL);
...
void Simple_Transmitter(void* params){
	...
	while(1){
		...
		if(sStatus != pdPASS){
			...
			HAL_UART_Transmit(&huart2, (uint8_t *)"true", 6, HAL_MAX_DELAY);
...
void Simple_Transmitter2(void* params){
	...
	BaseType_t sStatus;
	while(1){
		sStatus = xSemaphoreTake(sHandler2 , portMAX_DELAY);
