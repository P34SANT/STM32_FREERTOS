...
/*
 * task notification between 2 tasks
 *
*/
...
int main(void)
{
  ...
  xTaskCreate(Simple_Processor , "Simple task to processor" , SIMPLE_TASK_STACK_SIZE ,NULL , 1 , NULL);
  xTaskCreate(task1 , "Simple task" , SIMPLE_TASK_STACK_SIZE ,NULL , 1 , &task1handler);
...
void task1(void* params){
	...
	while(1){
		...
		if(ulTaskNotifyTake(pdFALSE , pdMS_TO_TICKS(1000)) != 0){
			...
			HAL_UART_Transmit(&huart2, (uint8_t *)"Not Receiving\n\r", 15, HAL_MAX_DELAY);
		}
